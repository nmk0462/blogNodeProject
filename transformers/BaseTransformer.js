...
class BaseTransformer {
    ...
    async getTimestamps() {
        ...
        if (syncData.created_at) {
            result = await App.helpers.cloneObj(result, {
                created_at: syncData.created_at,
                created_at_formatted: App.helpers.formatDate(syncData.created_at)
            })
        }
        if (syncData.modified_at) {
            result = await App.helpers.cloneObj(result, {
                modified_at: syncData.modified_at,
                modified_at_formatted: App.helpers.formatDate(syncData.modified_at)
            })
        }
        return result;
    }
    ...
    async getTransformedData(req, data, transformer, transformOptions = null) {
